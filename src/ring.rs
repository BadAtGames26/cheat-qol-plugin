...
impl ConfigBasicMenuItemSwitchMethods for RingSetting { 
    fn init_content(_this: &mut ConfigBasicMenuItem) {
        let _value = QOLCONFIG.lock().unwrap().ring;
    }

    extern "C" fn custom_call(this: &mut ConfigBasicMenuItem, _method_info: OptionalMethod) -> BasicMenuResult {
        let result = ConfigBasicMenuItem::change_key_value_i(QOLCONFIG.lock().unwrap().ring, 0, 4, 1);
        if QOLCONFIG.lock().unwrap().ring != result {
    ...
    extern "C" fn set_command_text(this: &mut ConfigBasicMenuItem, _method_info: OptionalMethod) {
        this.command_text = match QOLCONFIG.lock().unwrap().ring {
            1 => "C".into(),
            2 => "B".into(),
            3 => "A".into(),
            4 => "S".into(),
    ...
    extern "C" fn set_help_text(this: &mut ConfigBasicMenuItem, _method_info: OptionalMethod) {
        if QOLCONFIG.lock().unwrap().ring == 0 {
